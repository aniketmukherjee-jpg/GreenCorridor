...
const AICopilot = ({ missions }) => {
  ...
  return (
    <div className="fixed bottom-6 right-6 w-88 backdrop-blur-3xl bg-white/95 border border-blue-500/30 dark:bg-slate-900/90 dark:border-blue-500/40 glow-blue rounded-3xl shadow-2xl flex flex-col overflow-hidden z-[100] animate-in slide-in-from-bottom-8 transition-colors duration-300">
      <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-950/80">
        ...
        <span className="text-[10px] font-mono text-blue-600 dark:text-blue-400 bg-blue-500/10 px-2 py-0.5 rounded border border-blue-500/20">v3.2 LOGIC</span>
      </div>
      <div className="p-4 h-64 overflow-y-auto space-y-3 custom-scrollbar flex flex-col justify-end">
        {messages.map((msg, i) => (
          <div key={i} className="animate-in fade-in slide-in-from-bottom-2">
            <div className="p-3.5 rounded-2xl bg-blue-50 text-slate-900 border border-blue-200 dark:bg-slate-950/80 dark:border-blue-500/30 dark:text-slate-200 text-xs font-semibold leading-relaxed shadow-sm">
              {msg.text}
            </div>
          </div>
        ))}
        {isTyping && (
          <div className="text-blue-600 dark:text-blue-400 text-[11px] font-mono flex items-center space-x-2 p-2 bg-slate-100 dark:bg-slate-950/40 rounded-xl">
    ...
    <div className="w-full p-4 md:p-8 relative">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            <h1 className="text-3xl md:text-4xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-3">
              ...
              <span className="text-xs font-mono bg-blue-500/10 text-blue-600 border border-blue-500/20 dark:bg-blue-500/20 dark:text-blue-400 dark:border-blue-500/30 px-3 py-1 rounded-full uppercase tracking-widest">
                Fleet Hub
              </span>
            </h1>
          ...
          <button 
            ...
            className="px-6 py-3 rounded-2xl font-black text-sm text-white bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 transition-all shadow-[0_0_25px_rgba(59,130,246,0.3)] hover:scale-105 active:scale-95 flex items-center space-x-2"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          {[
            { label: 'Active Dispatches', val: activeCount, color: 'text-blue-600 dark:text-blue-400', glow: 'glow-blue border-blue-500/30' },
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          <div className="backdrop-blur-3xl bg-white/80 border border-slate-200 shadow-lg dark:bg-slate-900/60 dark:border-slate-800 dark:shadow-2xl rounded-3xl overflow-hidden">
            <div className="p-6 border-b border-slate-200 dark:border-slate-800/80 bg-slate-50 dark:bg-slate-950/60 flex items-center justify-between">
              ...
              <span className="text-[10px] font-mono text-blue-600 dark:text-blue-400 bg-blue-500/10 px-2.5 py-1 rounded-full border border-blue-500/20">{ambulances.length} UNITS TRACKED</span>
            </div>
              ...
              {ambulances.map(amb => (
                <div key={amb.id} className="flex items-center justify-between bg-slate-100/80 dark:bg-slate-950/60 p-3.5 rounded-2xl border border-slate-200 dark:border-slate-800 hover:border-slate-400 dark:hover:border-slate-700 transition-all">
                  <div className="flex items-center space-x-3">
                    <div className="w-10 h-10 bg-slate-200 dark:bg-slate-900 rounded-xl border border-slate-300 dark:border-slate-800 flex items-center justify-center text-lg">
                      🚑
                    </div>
                    <div>
        ...
        <div className="backdrop-blur-3xl bg-white/80 border border-slate-200 shadow-lg dark:bg-slate-900/60 dark:border-slate-800 dark:shadow-2xl rounded-3xl overflow-hidden">
          <div className="p-6 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/60 flex justify-between items-center">
            <h2 className="text-xl font-black text-slate-900 dark:text-white">Active & Recent Missions</h2>
                    ...
                    >
                      ...
                      <td className="p-5">
                        <span className={`flex items-center space-x-1.5 px-3 py-1 rounded-full text-[10px] uppercase font-black tracking-wider w-max ${
                          ...
                          mission.status === 'IN_PROGRESS' ? 'bg-blue-500/10 text-blue-600 border border-blue-500/30 dark:bg-blue-500/20 dark:text-blue-400 dark:border-blue-500/40' :
                          ...
                          {mission.status === 'IN_PROGRESS' && <span className="w-1.5 h-1.5 bg-blue-500 dark:bg-blue-400 rounded-full animate-ping"></span>}
              ...
              <form onSubmit={handleNewMission} className="space-y-4">
                <div>
                  ...
                  <select 
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 rounded-xl px-4 py-2.5 text-xs text-slate-900 dark:text-white outline-none focus:border-blue-500"
                  >
                ...
                <div>
                  ...
                  <input required name="hospital" type="text" className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 rounded-xl px-4 py-2.5 text-xs text-slate-900 dark:text-white outline-none focus:border-blue-500" defaultValue="Victoria Hospital" />
                </div>
                ...
                <div>
                  ...
                  <select name="priority" className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 rounded-xl px-4 py-2.5 text-xs text-slate-900 dark:text-white outline-none focus:border-blue-500">
                      ...
                      return (
                        <div 
                          ...
                          className={`p-3 rounded-xl border transition-all flex flex-col ${
                            ...
                            isSelected 
                              ? 'bg-blue-50 border-blue-500 dark:bg-blue-500/20 dark:border-blue-500 shadow-md cursor-pointer' 
                ...
                <div className="pt-4 flex justify-end space-x-3">
                  <button type="button" onClick={() => setIsModalOpen(false)} className="px-4 py-2 rounded-xl text-xs font-bold text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white">Cancel</button>
          ...
          <div className="fixed inset-0 z-[9999] flex justify-end">
            ...
            <div className="relative w-full max-w-sm h-full bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-800 p-6 flex flex-col z-10 animate-in slide-in-from-right duration-300">
              <div className="flex justify-between items-center border-b border-slate-200 dark:border-slate-800 pb-4 mb-6">
                ...
                <button onClick={() => setSelectedMission(null)} className="text-slate-400 hover:text-slate-900 dark:hover:text-white font-bold text-2xl">&times;</button>
              </div>
